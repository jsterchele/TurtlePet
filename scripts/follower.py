...
if __name__ == "__main__":
    ...
    # Get the stopping distance and followDistance from the parameter server.  The
    # default is 2.  Tell us what value we got, just so we know.
    followDistance = rospy.get_param('follower/followDistance', 2)
    stopDistance = rospy.get_param('follower/stopDistance', 0.6)
    maxSpeed = rospy.get_param('follower/maxSpeed', .2)
    
    #goal default locations
    rospy.loginfo('Starting node with a stopping distance of {0} and followDistance of {1}'.format(stopDistance, followDistance))

    # Start up a stopper
    follower = follower(followDistance, stopDistance, maxSpeed)
