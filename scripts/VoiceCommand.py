...
# Global variables for following and wandering commands.
wander = False
follow = False
...
class voice_cmd_vel:
	...
	if wander is True and follow is False:
            ...
            topIndex = int(math.ceil(midIndex + midIndex * .25))
            btmIndex = int(math.floor(midIndex - midIndex * .25))
            data.ranges = data.ranges[btmIndex:topIndex]
            #removing all of the NaNs from the scan data.
	...
	# spin commands
        ...
        # Sing command. plays one of a few songs.
        elif msg.data.find("sing") > -1 : 
            c = random.choice(seq)
            rospy.loginfo(c)
            self.sound_client.playWave(c)
        # lay down command. Plays the low battery sound.
